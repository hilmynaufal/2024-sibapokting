        ...
        <div class="card mb-5 mb-xl-8">
            <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bold fs-3 mb-1">Tambah Master Kios Pupuk</span>
                    </h3>
            </div>
            <div class="card-body" style="text-align:left;">
                <form class="form-horizontal" wire:submit="create">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-10">
                                <div class="input-group mb-2" wire:ignore>
                                    <select x-init="$($el).select2({ placeholder: '-- Pilih Distributor --', });
                                $($el).on('change', function() {
                                    $wire.set('id_distributor', $($el).val());
                                })" wire:model="id_distributor" name="id_distributor" id="id_distributor"
                                        class="form-control form-control-lg form-select-solid @error('id_distributor') is-invalid @enderror">
                                        <option value="">-- Pilih Distributor --</option>
                                        @foreach($distributorList as $distributor)
                                        <option value="{{$distributor->id}}">{{strtoupper($distributor->namadistributor)}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="form-group mb-3 fv-row fv-plugins-icon-container">
                        <div class="row">
                            <div class="col-md-2">
                                <label class="form-label">Nama Kios Pupuk<span class="text-danger">*</span></label>
                            </div>
                            <div class="col-md-10">
                                <input type="text" class="form-control @error('namapupuk') is-invalid @enderror" name="namapupuk"
                                    wire:model="namapupuk" id="namapupuk">
                                @error('namapupuk') <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-footer">
                        <a class="btn btn-secondary" href="{{route('master.referensi.pupuk')}}">Close</a>
