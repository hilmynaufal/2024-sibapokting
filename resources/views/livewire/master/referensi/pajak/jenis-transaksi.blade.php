...
@stop
@section('menu')
Referensi > Pajak > <b>Jenis Transaksi</b>
@stop
<!--begin::Col-->
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                        ...
                        <thead>
                            <tr class="text-uppercase">
                                <th class="pl-0" style="min-width: 100px">Kode Jenis Transaksi 
                                </th>
                                <th class="pl-0" style="min-width: 100px">Nama Jenis Transaksi 
                                    @if ($sortColoumName === 'kd_jenis_transaksi')
                                    <span wire:click="sortBy('kd_jenis_transaksi')" class="float-end text-sm"
                                        style="cursor: pointer;">
                                        <i
                                            class="{{ $sortColoumName === 'kd_jenis_transaksi' && $sortDirection === 'desc' ? ' fas fa-sort-up ' : 'fas fa-sort-down' }}"></i>
                                    </span>
                                    @else
                                    <span wire:click="sortBy('kd_jenis_transaksi')" class="float-end text-sm"
                                        style="cursor: pointer;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <span class="text-center text-gray-80 font-weight-bolder text-hover-success font-size-lg">{{ $item->kd_jenis_transaksi}}</span>
                                </td>
                                <td>
                                    <span class="text-center text-gray-80 font-weight-bolder text-hover-success font-size-lg">{{ $item->nm_jenis_transaksi}}</span>
                                </td>
                                <td>
                ...
                <div class="card card-flush py-4">
                    ...
                    <div class="card-body pt-0">
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Kode Jenis Transaksi</label>
                            <input type="hidden" class="form-control  @error('id_jenis_transaksi') is-invalid @enderror" wire:model="id_jenis_transaksi" />
                            <input type="text" class="form-control  @error('kd_jenis_transaksi') is-invalid @enderror" wire:model="kd_jenis_transaksi" />
                            @error('kd_jenis_transaksi')
                            <div class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback">
                                {{ $message }} </div>
                            @enderror
                        </div>
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Nama Jenis Transaksi</label>
                            <input type="text" class="form-control  @error('nm_jenis_transaksi') is-invalid @enderror" wire:model="nm_jenis_transaksi" />
