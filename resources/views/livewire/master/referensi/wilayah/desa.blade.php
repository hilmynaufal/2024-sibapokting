                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                        ...
                        <thead>
                            <tr class="text-uppercase">
                                <th class="pl-0" style="min-width: 100px">Kode Desa
                                </th>
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container" wire:ignore>
                            ...
                            @if ($mode=='create' ) 
                                <select x-init="$($el).select2({ placeholder: '-- Pilih Kecamatan --', });
                                $($el).on('change', function() {
                                    $wire.set('id_kec', $($el).val())
                                })" wire:model.live="id_kec"  name="id_kec" id="id_kec" class="form-control form-control-lg form-select-solid">
                                <option value=""></option>
                                    @foreach($kecamatan_list as $kecamatan)
                                    <option value="{{$kecamatan->id}}">{{$kecamatan->name}}</option>
                                    @endforeach
                            ...
                            @else
                                ...
                                $($el).on('change', function() {
                                    $wire.set('id_kec', $($el).val())
                                })" wire:model.live="id_kec"  name="id_kec" id="id_kec" disabled class="form-control form-control-lg form-select-solid">
                                <option value=""></option>
                                    @foreach($kecamatan_list as $kecamatan)
                                    <option value="{{$kecamatan->id}}">{{$kecamatan->name}}</option>
                                    @endforeach
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Kode Desa</label>
                            @if ($mode=='create' ) 
                            <input type="text" class="form-control  @error('id_desa') is-invalid @enderror"
                                placeholder="Ex: 3204102001" wire:model="id_desa" />
                            @else
                            <input type="text" disabled class="form-control  @error('id_desa') is-invalid @enderror"
                                placeholder="Ex: 3204102001" wire:model="id_desa" />
                            @endif
                            @error('id_desa')
                            <div
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            ...
                            <input type="text" class="form-control  @error('name') is-invalid @enderror"
                                placeholder="Ex: Soreang" wire:model="name" />
                            @error('name')
                            <div
                                class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback">
                                {{ $message }} </div>
