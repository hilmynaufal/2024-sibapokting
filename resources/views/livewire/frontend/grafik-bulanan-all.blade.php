        ...
        <div class="col-xl-12">
            <div class="card card-flush">
                ...
                <div class="card-header py-5">
                    <!--begin::Title-->
                    <h3 class="card-title fw-bold text-gray-800">PERKEMBANGAN HARGA {{getKomoditas($komoditas)->namakomoditas}} SEMUA PASAR</h3>
                    <!--end::Title-->
...
<script>
    ...
    $(document).ready(function () {
        ...
        var settings1 = {
            "url": {!! json_encode(url('/')) !!}+"/api/komoditasLineDashAll?pasar="+$('#pasar').val() +"&komoditas="+$('#komoditas').val() +"&date="+$('#date_komoditas').val(),
            "method": "GET",
            "timeout": 0,
        };
        // console.log(settings1);
        $.ajax(settings1).done(function (response) {
            const updatedSeries = Object.keys(response.price_before).map(pasar => {
                return {
                    name: pasar,
                    data: response.price_before[pasar]
                };
            });

            chart2.updateSeries(updatedSeries);
        ...
        // changeTabel();
        // const series = Object.keys(data.price_before).map(pasar => {
        //     return {
        //         name: pasar,
        //         data: data.price_before[pasar]
        //     };
        // });
