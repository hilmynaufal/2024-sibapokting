...
class ObjekPajak extends Component
{
    ...
    {
        return view('livewire.main.layanan.bphtb.form.objek-pajak');
    }
    ...
    #[\Livewire\Attributes\On('update-lat')]
    public function updatedLat($val)
    {
        $this->latitude = $val;
    }
    
    #[\Livewire\Attributes\On('update-lng')]
    public function updatedLng($val)
    {
        $this->longitude = $val;
    }
    ...
    public function create()
    {
            ...
            if($model->save()){
                ...
                // Menghapus persyaratan dengan jenis_transaksi_id yang berbeda
                $persyaratan = Persyaratan::where('id_bphtb', $model->id_bphtb)
                ->where('jenis_transaksi_id', '!=', $model->jenis_transaksi_id)
                ->delete();
                
                $refPersyaratan = RefPersyaratan::where('jenis_transaksi_id', $model->jenis_transaksi_id)->get();
                
                foreach($refPersyaratan as $item) {
                    Persyaratan::updateOrCreate([
                        'id_bphtb' =>  $model->id_bphtb,
                        'jenis_transaksi_id' =>  $model->jenis_transaksi_id,
