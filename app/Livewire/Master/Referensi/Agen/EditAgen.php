...
class EditAgen extends Component
{
    use LivewireAlert;
    ...
    {
        return view('livewire.master.referensi.agen.edit-agen');
    }
    
    public function mount($id)
    {
        $idAgen = Crypt::decrypt($id);
        $model = Model::where('id',$idAgen)->first();
        $this->id_agen           = $model->id;
        $this->namaagen            = $model->namaagen;
    ...
    public function create()
    {
        $this->validate([
            ...
            $model = Model::where('id',$this->id_agen)->first();
            $model->namaagen    = $this->namaagen;
            ...
            if($model->update()){
                $this->alert('success', 'Data Agen Berhasil di Ubah', [
                    'position' => 'top',
                    'timer' => 3000,
                    'toast' => true,
                    'timerProgressBar' => true,
                ]);
                return redirect()->route('master.referensi.agen');
            }else{
                $this->alert('error', 'Data Agen Gagal di Ubah', [
                    'position' => 'top',
                    'timer' => 3000,
                    'toast' => true,
                    'timerProgressBar' => true,
