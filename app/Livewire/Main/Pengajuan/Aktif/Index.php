<?php
namespace App\Livewire\Main\Pengajuan\Aktif;
...
class Index extends Component
{
    ...
    {
        setActivity('Pengguna Mengakses Halaman SPPD');
        
        if(Auth::user()->role_id==1){
            $model = Model::where('deleted_at',null)
            ->whereRaw('LOWER(nama_wp) like ?', ['%'.strtolower($this->search).'%'])
            ->paginate($this->perpage);
        }else{
            $model = Model::where('deleted_at',null)->where('created_id',Auth::user()->id)
            ->whereRaw('LOWER(nama_wp) like ?', ['%'.strtolower($this->search).'%'])
            ->paginate($this->perpage);
        }
        // dd($model);
        
        return view('livewire.main.pengajuan.aktif.index',
        [
            'model' => $model
        ]);
    }
