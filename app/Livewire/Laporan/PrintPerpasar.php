...
class PrintPerpasar extends Component
{
    ...
    public function mount($pasar_tabel,$end){
        $dt = new \Carbon\Carbon(now());
        $tanggal = $dt->format('Y-m-d');
        $this->start = date('Y-m-d',strtotime($end . "-1 days"));
        $this->end = date('Y-m-d',strtotime($end));
        $this->pasar_tabel = $pasar_tabel;
    ...
    public function render()
    {
        ...
        return view('livewire.laporan.perpasar-print', [
          'model'=> $data
        ]);
    }
